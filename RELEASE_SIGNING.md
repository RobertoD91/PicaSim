...
There is also an unsigned monthly release workflow —
`.github/workflows/github-release.yml` (rolling pre-release with Linux
AppImages + Windows zip + Android APK; only the 3 most recent are kept) —
which needs no signing material: it is enabled by creating the repository
**variable** (not secret) `PICASIM_PUBLISH_RELEASES` with value `true` under
**Settings → Secrets and variables → Actions → Variables**, or via CLI:
