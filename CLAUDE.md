...
- `macos-release.yml` - Monthly signed + notarized universal DMG, published as a rolling pre-release (keeps the 3 most recent)
- `github-release.yml` - Monthly multi-platform rolling pre-release (Linux AppImages, Windows zip, Android APK; keeps the 3 most recent)
- `official-release.yml` - Manual-only full release `v<version>` with all platforms; the macOS DMG is signed + notarized when the signing secrets are present, unsigned otherwise
...
The three monthly release workflows (`ios-testflight.yml`, `macos-release.yml`, `github-release.yml`) run on a monthly cron plus `workflow_dispatch`; the two that publish GitHub releases publish them as pre-releases and prune all but the 3 most recent. They do not hardcode a repository name: the two signed ones gate on a `check-secrets` job that verifies the required signing secrets are configured, and `github-release.yml` gates on the repository variable `PICASIM_PUBLISH_RELEASES` being `true` — so all three are a no-op on forks that have not opted in. `official-release.yml` is `workflow_dispatch`-only and publishes the proper `v<version>` release marked as Latest (an `overwrite` input allows replacing an existing one). See `RELEASE_SIGNING.md` for how to create the signing material and upload the secrets. TestFlight uploads need a strictly increasing `CFBundleVersion`, provided via the `PICASIM_IOS_BUILD_NUMBER` CMake cache variable (set to the workflow run number in CI).
